...
from consumer import find_pattern_in_windowStream, find_pattern_in_windowBatch, consumer_done, consumer_temp, consumer_fin
from consumer import findAverageBatch, findMax, findMini, findStd, findStdStream
...
def start_system(regex_pattern, genProb, duration, throughput_per_sec):
    ...
    # consumer_thread = threading.Thread(target=findStd, args = ("thread_1", 1, duration))
    ...
    #             future = executor.submit(findStdStream, thread_name, i, i+maxWindowSize-1)
    #             i = i+10
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        while not finish.is_set():
            if tempsecond.is_set():
                ...
                thread_name = f"consumer_thread_{i}"
                # Submit the task to the thread pool
                future = executor.submit(find_pattern_in_windowStream, regex, bounds, thread_name, i, i+maxWindowSize-1)
                i = i + 1
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers = 1) as executor:
        if tempsecond.is_set():
            ...
            tempsecond.clear()
            # Submit the task to the thread pool
            future = executor.submit(find_pattern_in_window, regex, bounds, thread_name, i, i+maxWindowSize-1)
            i = i + 1
            result = future.result()
            key = int((int(result[1].split("_")[2])+maxWindowSize-1)/10)
            if((int(result[1].split("_")[2])+maxWindowSize-1)%10 == 0): key = key-1
...
if __name__ == '__main__':
    ...
    start_system(regex_pattern=regex_pattern, genProb=0.5, duration=duration, throughput_per_sec = throughput_per_sec)        
